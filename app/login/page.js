...
export default function Home() {
  ...
  // 登录态检查中：session 接口返回前渲染纯色加载页（与聊天页加载页一致），
  // 不渲染登录表单，避免已登录用户访问 / 时先闪现登录界面
  ...
  // 已经登录的话直接进聊天页（客户端跳转，不刷新）；确认未登录才显示登录表单
  useEffect(() => {
    let alive = true;
  ...
  // 登录态检查中：渲染与聊天页一致的纯色加载页，不渲染登录表单（所有 hooks 之后才可 early return）
  if (checking) {
    return (
      <div className="h-screen flex items-center justify-center bg-[#fafaf8] text-slate-800">
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen bg-[#fafaf8] text-slate-800">
      {/* 左上角：回首页（未登录用户也能从这里回到宣传页） */}
      <a
        href="/home"
        className="absolute left-6 top-6 text-xs text-slate-400 transition-colors duration-150 hover:text-slate-600"
      >
        ← 返回首页
      </a>
      <h1 className="font-display text-6xl font-bold tracking-tight mb-2">Solace</h1>
