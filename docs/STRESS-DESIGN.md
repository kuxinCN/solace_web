...
| **词表已改由后台管理**（2026-09） | ⚠️ `CRISIS_PATTERNS` 和 `NEGATIVE_WORDS` **不再直接决定行为** —— 它们变成 `lib/lexicon-store.js` 里的 `DEFAULT_GROUPS`（**只用于首次播种**），实际生效的是 `keyword_groups` 表。管理员在后台「内容安全与压力 → 统一词表」改完，调一次 `refreshLexicons()` 重编译就生效（词表**预编译在内存里**，不在每条消息上读库）。下面 4.1 节贴的 `NEGATIVE_WORDS` 内容仍然准确，但请以"**它只是默认值**"为前提读。 |
| **危机词多了「构词排除」** | `DEATH_COMPOUND`（18 个构词字）：命中片段**后面紧跟**这些字（`死磕` / `死线` / `死记` / `死党`）就说明那个「死」是词头，**不算危机**。⚠️ 这张表故意做窄 —— 「我想死」和「我想死磕」只差一个字，必须一个命中、一个不命中。 |
