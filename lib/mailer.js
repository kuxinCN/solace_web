 ...
 *
 * 可选覆盖项（用于非登录场景，如找回密码 purpose="reset"）：
 *   ttlSeconds / cooldownSeconds / emailIntro / emailSubject
 * 不传时一律走后台「login」配置，登录链路行为不变。
 */
export async function issueEmailCode({
  ...
  const cooldown =
    cooldownSeconds != null
      ...
      : Number(loginConfig.sendCooldownSeconds) || 0;
  ...
  const ttl =
    ttlSeconds != null ? Number(ttlSeconds) : Number(loginConfig.codeTtlSeconds) || 300;
  const expiresAt = new Date(Date.now() + ttl * 1000);

  // 找回密码等场景用专属文案（"你正在重置密码"而不是"你正在登录"）
  const mailConfig =
    emailIntro || emailSubject
      ? {
          ...loginConfig,
          emailIntro: emailIntro || loginConfig.emailIntro,
          emailSubject: emailSubject || loginConfig.emailSubject,
        }
      : loginConfig;

  // 先把邮件发出去，成功之后再入库：
  // 否则发信失败（比如 SMTP 没配好）也会留下一条记录，占掉每日额度、并把用户卡在冷却里
  const mail = buildCodeMail(code, mailConfig, siteConfig);
