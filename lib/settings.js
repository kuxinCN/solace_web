...
"""`,
    ...
    minMessages: 4,
    // 一次最多取多少条聊天消息（取最近的那些）
    // ⚠️ 从 120 降到 60：**输入越长，留给"输出"的额度就越少** ——
    //    而生成日记的输出本来就长（一篇两三百字），输入太长会把输出挤没，
    //    表现就是"日记写到一半戛然而止"。
    maxMessages: 60,
    // 聊天记录拼起来最长多少字（超出就取最近的部分）
    // ⚠️ 从 6000 降到 2500：同上。
    maxChars: 2500,

    /* ---- 输出额度（走批量推理时，每条请求允许模型输出多少 token）---- */

    // ⚠️ **最常需要调的就是这个**：
    //    输入（聊天记录）一长就会挤占输出预算，给少了模型的日记会"写到一半戛然而止"。
    //    出过的事：一个用户 4000+ 字的聊天记录 + 提示词要求写 150-400 字 →
    //    输出被掐断，而**掐出来的半截内容仍然符合格式**，任务还标成了 pass。
    //    现在被截断的结果会直接标 failed（不保存半截），原因里会写清输入/输出字数。
    //    调大这个值就能解决 —— **保存即生效，不用改代码、不用重新部署**。
    ...
    // ⚠️ 会替换提示词里的 `{minWords}` / `{maxWords}` 两个占位符 ——
    //    所以**改这两个数就够了，不用去动提示词正文**。
    //    别设太长：输出越长越容易被额度掐断，而且用户读日记不是读作文。
    diaryMinWords: 150,
    diaryMaxWords: 250,
...
- 语气自然，像随手写下的，不是作文
- **篇幅控制在 {minWords}-{maxWords} 字之间**，分 1-2 段
  （这两个数来自后台「日记」页的「日记字数」设置，不用改这里）
- ⚠️ **一定要写完整**：最后一句必须是完整的句子，不要写到一半就停下来
...
"""`,
  ...
  review: {
    ...
    timeoutSeconds: 120,
    // ⚠️ 走批量推理时，**每条请求允许模型输出多少 token**。
    //    内容审核只需要回一个极短的 JSON，200 足够；给多了纯属浪费。
    //    （生成日记那种长输出的额度在「日记」页调，不在这里）
    reviewMaxTokens: 200,
...
{diary}`,
  ...
  safety: {
    ...
    stressEnabled: false,
    // 弹出提醒的阈值（0-100）。
    // ⚠️ 这是**默认值**：用户自己没调过时用它，调过就用他调的那个（见 resolveThreshold）。
    //    默认 65 而不是 70 —— 本地规则很难把分数推到 70 以上（基准 50，
    //    得同时出现「绝望」「崩溃」这个量级的词才够），那样会出现"压力很大却从不提醒"。
    stressThreshold: 65,
...
const NUMBER_RANGES = {
  review: {
    ...
    keepDays: [1, 365],
    // ⚠️ 输出额度：太小必然截断，太大有些上游会直接拒
    reviewMaxTokens: [16, 16000],
  },
  // ⚠️ diary 分组以前没有范围校验，这次一起补上 ——
  //    这几个都是"给少了会出问题"的数值（尤其输出额度）
  diary: {
    generateHour: [0, 23],
    minMessages: [1, 500],
    maxMessages: [1, 500],
    maxChars: [200, 40000],
    generateMaxTokens: [16, 16000],
    moodMaxTokens: [16, 16000],
  },
  ai: {
    temperature: [0, 2],
    maxTokens: [1, 8192],
