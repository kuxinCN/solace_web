 ...
 * 给内容算一个短指纹（FNV-1a）。
 *
 * ⚠️ 用途只是"判断这份内容是不是已经在队列里了"，**不是安全用途**，
 *    所以不追求抗碰撞 —— 万一撞了，最坏结果是**少排一条任务**，
 *    总比"同一份内容排两条、生成两篇一模一样的日记"要好。
 *
 * ⚠️ 故意**不用 `node:crypto`**：那个模块会牵进 Node 内置依赖，
 *    而这个仓库在 Next 的构建边界上已经踩过一次类似的坑（见 AGENTS.md 的红线）。
 */
...
/**
 * 算一条任务的去重键 —— **同一份"内容"在队列里只应该有一条活跃任务**。
 *
 * ⚠️ 出过的事：先点「重试失败任务」（把旧的 failed 打回队列），又点
 *    「为昨天排生成任务」，结果**同一天生成了两篇一模一样的日记**。
 *    因为旧的排队逻辑只看 `diaries` 表（上次生成失败时那儿根本没记录），
 *    而这里从来不查重。
 *
 * 规则：
 *   · 内容审核   `review:{userId}:{字段}:{内容指纹}`
 *   · 日记打标   `diary_mood:{diaryId}:{内容指纹}`  ← 改了正文指纹就变，所以能重新排队
 *   · 生成日记   `diary_generate:{userId}:{日期}`
 *
 * ⚠️ **生成日记刻意不用内容指纹**，而是用「用户 + 日期」：
 *    它的输入是那天的聊天记录，而聊天记录**会一直变长**（用户还在聊），
 *    用指纹的话每多聊一句就换一个 key，去重等于没做。
 *
 * @param {object} params
 * @param {string} [params.provided] 调用方直接给的 key（生成日记用它传日期）
 */
function buildDedupeKey({ kind, userId, field, targetId, content, provided }) {
  if (provided) return String(provided).slice(0, 191);

  if (kind === "diary_generate") {
    // ⚠️ 正常情况下调用方（`createDailyDiaryTasks`）会传带日期的 key。
    //    这里的兜底也**必须按天** —— 绝对不要用一个固定字符串（比如 `unknown`）：
    //    那样**所有生成任务会共用同一个 key**，前一天那条还没跑完时，
    //    今天的新任务会被查重挡掉，而调用方只拿到一个 `false` —— **静默丢失**。
    const day = new Date(Date.now() + 8 * 3600 * 1000).toISOString().slice(0, 10);
    return `diary_generate:${userId}:${day}`;
  }
  if (kind === "diary_mood") {
    return `diary_mood:${targetId || 0}:${contentFingerprint(content)}`;
  }
  return `review:${userId}:${field || ""}:${contentFingerprint(content)}`;
}
...
export async function enqueueReview({
  ...
  const dedupeKey = buildDedupeKey({
    ...
    targetId,
    content: value,
    provided: providedDedupeKey,
  });

  try {
    // ⚠️ **查重**：同一个 key 只要还有"活跃任务"（排队中 / 已提交 / 等人工），
    //    就不再入队。这样反复点「为昨天排任务」、或者「重试失败任务 + 再排一次」
    //    这类操作都不会堆出重复任务。
    //    ⚠️ 已经跑完（pass / failed）的不算活跃 —— 失败了要允许重来。
    try {
      const existing = await query(
        `SELECT id FROM content_review_tasks
          WHERE dedupe_key = ? AND status IN ('pending','submitted','manual')
          LIMIT 1`,
        [dedupeKey]
      );
      if (existing.length) return false;
    } catch {
      /* 老库还没补 dedupe_key 列时跳过查重，不因此阻塞入队 */
    }

    await execute(
      `INSERT INTO content_review_tasks
         (user_id, task_kind, field, target_id, content, is_image, status, dedupe_key)
       VALUES (?, ?, ?, ?, ?, ?, 'pending', ?)`,
      [userId, kind, meta.field, targetId || null, value, meta.isImage ? 1 : 0, dedupeKey]
    );
...
function buildMessages(task, config) {
  ...
  //    生成出来后，日记正文来自 AI 的返回。
  if (kind === "diary_generate") {
    // ⚠️ 占位符替换：把后台配的"日记字数"填进提示词。
    //
    //    这样"日记写多长"可以在后台直接调，而**不用去改提示词正文** ——
    //    改正文的风险大得多：输出格式（标签 / 标题 / 正文三个行首标记）
    //    就是靠提示词约定的，动它容易把解析弄坏。
    const minWords =
      Number(config.diaryMinWords) > 0 ? Math.round(Number(config.diaryMinWords)) : 150;
    const maxWords =
      Number(config.diaryMaxWords) > 0 ? Math.round(Number(config.diaryMaxWords)) : 250;

    const prompt = String(config.diaryGeneratePrompt || "")
...
export async function applyVerdict({ taskId, verdict, reason, provider }) {
  ...
  if (kind === "diary_generate") {
    ...
    // ⚠️ **双保险**：真正写日记之前再查一次"这一天是不是已经有一篇 AI 日记了"。
    //
    //    队列那边已经有去重（`dedupe_key`），但两道防线不嫌多 ——
    //    这个 bug 的表现是"用户看到两篇一模一样的日记"，代价太直观了。
    //    真撞上了就把这条任务标成 stale，不写第二篇。
    try {
      const existing = await query(
        `SELECT id FROM diaries
          WHERE user_id = ? AND source = 'ai'
            AND diary_date = DATE_SUB(CURDATE(), INTERVAL 1 DAY)
          LIMIT 1`,
        [task.user_id]
      );
      ...
      if (existing.length) {
        await execute(
          ...
          [`这天的 AI 日记已经存在（#${existing[0].id}），跳过重复生成`, now(), taskId]
        );
        return { ok: true, verdict: "duplicate", field: "diary_generate", label: "AI 日记" };
      }
    } catch {
      /* 查不了就继续写；队列那道防线还在 */
    }
    ...
    await execute(
      ...
      [
        `已生成日记（输入 ${String(task.content || "").length} 字 / 输出 ${String(
          draft.content
        ).length} 字）：${String(draft.title || "").slice(0, 40)}`,
        String(provider || ""),
        now(),
        taskId,
      ]
  ...
  if (kind === "diary_mood") {
    const diaryConfig = await getGroup("diary");
    const fallback = String(diaryConfig?.defaultMood || "").trim() || "说不清";

    // ⚠️ **白名单兜一层**（这里出过事故，别省）。
    //
    //    以前是把传进来的 verdict 直接写进 diaries.mood。结果：管理员在「数据审核」页
    //    对一条日记打标任务点了「驳回」，`applyManualVerdict` 传进来的是字符串
    //    `"reject"` —— 它被当成了"情绪词"落库，用户的日记卡片上就出现了一个
    //    「reject」。
    //
    //    打标根本没有"通过 / 驳回"的概念。**凡是落到 8 个情绪词之外的值，
    //    一律当成"这次没打出有效标签"**，退回后台配的默认标签。
    const mood = pickMoodFromText(String(verdict || "")) || fallback;
 ...
 * 自动重试上限。
 *
 * ⚠️ 失败的会被**重新捞回来**（用户要求："状态为失败的再次放入待处理池"），
 *    但要防无限重试 —— 到上限就永久停在 failed，等人工处理。
 *    第 1 次失败 → 下次提交自动重来；第 2 次还失败 → 就不再自动捞了。
 */
...
async function takePending(limit, config) {
  void config;
  return query(
    // ⚠️ 除了 pending，**还捞"失败但还没重试够次数"的** —— 这样一次上游抖动
    //    （超时、额度截断、临时 500）不会让任务永久卡死，下一次自动提交就带上了。
    //    后台「日记」页的「重试失败任务」按钮则是把 retry_count 清零，
    //    让管理员能手动再推一次（用于换过提示词 / 换过模型之后）。
    `SELECT id, user_id, task_kind, field, target_id, content, is_image, status, retry_count
       FROM content_review_tasks
      WHERE status = 'pending'
         OR (status = 'failed' AND COALESCE(retry_count, 0) < ${MAX_AUTO_RETRY})
      ORDER BY id ASC
      LIMIT ?`,
    [limit]
  );
...
export async function submitPendingTasks({ limit } = {}) {
  ...
  try {
    ...
    // ⚠️ 生成日记的**输出额度**也在这里带上（后台「日记」页可调）。
    //    这是最需要调的一个数：给少了就会"写到一半戛然而止"。
    config.diaryGenerateMaxTokens = Number(diaryConfig?.generateMaxTokens) || 0;
    config.diaryMoodMaxTokens = Number(diaryConfig?.moodMaxTokens) || 0;
    // 日记正文的字数要求（会替换提示词里的 {minWords} / {maxWords}）
    config.diaryMinWords = Number(diaryConfig?.diaryMinWords) || 0;
    config.diaryMaxWords = Number(diaryConfig?.diaryMaxWords) || 0;
  } catch {
    /* 读不到就走内置默认额度 */
  }
  ...
  // ⚠️ 把"捞回来的失败任务"记一次重试（防无限重试）。
  //
  //    配合取队列时的 `retry_count < MAX_AUTO_RETRY` 条件，形成一条闭环：
  //      失败 → 下次自动提交时重来 → 再失败就到上限 → 之后只有管理员
  //      在后台点「重试失败任务」才会再动它。
  //    这样"上游抖一下"（超时、额度截断、临时 500）不会让任务永久卡死，
  //    而"内容本身就有问题"的任务也不会被无限重跑烧钱。
  const retryIds = pending
    .filter((task) => String(task.status || "") === "failed")
    .map((task) => task.id);
...
async function submitAsBatch({ tasks, config }) {
  ...
  // ⚠️ 输出额度**按任务类型给** —— 生成日记必须给足。
  //
  //    这段历史值得记一下：批量请求一开始是**不带 max_tokens** 的
  //    （当时想的是"照官方模板一字不改，最不容易被校验拒掉"）。
  //    审核（输出一个 JSON）和打标（输出一个词）确实够用，
  //    但**生成日记要输出一整篇 150-400 字的日记** ——
  //    上游的默认额度一截，返回的 content 就是空字符串，表现成
  //    「生成结果为空或格式不对：AI 生成：」（冒号后面什么都没有），
  //    而且从错误信息里**完全看不出原因**。
  const taskKind = String(tasks[0]?.task_kind || "review");

  /**
   * 输出额度的**兜底默认值** —— 后台可以覆盖（这才是常态）。
   *
   * ⚠️ 这些值为什么做成可配：
   *    输入（聊天记录）一长就会挤占输出预算，而**"写到一半被掐断"这种事
   *    只能靠现场调额度解决** —— 为了改一个数字去走一遍"改代码 → 打包 → 上传 → 构建"
   *    太慢了，而且每次遇到不同长度的输入都可能要重调。
   *
   * 后台在哪调：
   *   · 内容审核     → 「数据审核」页 → 单条输出上限
   *   · 日记情绪打标 → 「日记」页 → 打标输出上限
   *   · AI 生成日记  → 「日记」页 → **生成长度上限**（最常需要调的就是这个）
   */
  const MAX_TOKENS_FALLBACK = {
    review: 200,
    diary_mood: 30,
    diary_generate: 3000,
  };

  // ⚠️ 后台配的值优先；没配或配成 0 才用兜底。
  //    上下限也夹一下：太小必然截断，太大有些上游会直接拒。
  const configuredMaxTokens =
    taskKind === "diary_generate"
      ? Number(config.diaryGenerateMaxTokens) || 0
      : taskKind === "diary_mood"
        ? Number(config.diaryMoodMaxTokens) || 0
        : Number(config.reviewMaxTokens) || 0;

  const maxTokens =
    configuredMaxTokens > 0
      ? Math.min(Math.max(Math.round(configuredMaxTokens), 16), 16000)
      : MAX_TOKENS_FALLBACK[taskKind] || MAX_TOKENS_FALLBACK.review;
  // 审核和打标要的是稳定（同样的输入给同样的输出），
  // 生成日记要的是一点变化 —— 不然每篇 AI 日记都长一个样，
  // 用户很快就会发现"这不是在写我"
  const TEMPERATURE_BY_KIND = {
    review: 0,
    diary_mood: 0,
    diary_generate: 0.8,
  };
  ...
  const temperature =
    TEMPERATURE_BY_KIND[taskKind] != null ? TEMPERATURE_BY_KIND[taskKind] : 0;

  // ① 组装 JSONL。格式严格按小米文档：
  //    {"custom_id":"request-1","method":"POST","url":"/v1/chat/completions","body":{...}}
  //    —— custom_id 在文件内必须唯一，我们用任务 id，回收结果时靠它对齐。
  //
  // ⚠️ 如果创建批次因为 max_tokens / temperature 被拒（HTTP 500 / 400），
  //    错误信息里会点名这两个字段 —— 那是唯一需要怀疑的地方。
...
async function applyBatchResults({ batchId, outputFileId, config, batchBase }) {
  ...
  for (const line of text.split("\n")) {
    ...
    // ⚠️ 除了 content，还要把 **finish_reason** 和 token 用量取出来 ——
    //    否则"AI 返回为空"这种事根本查不出原因（错误信息里只有一个空冒号）。
    //    实测最有用的两类值：
    //      · finish_reason = "length"          → **被 max_tokens 截断**（生成日记最常踩）
    //      · finish_reason = "content_filter"  → 被内容过滤
    //      · finish_reason = "stop" 但内容是空 → 模型真的什么都没说
    const choice = item?.response?.body?.choices?.[0] || null;
    const reply = choice?.message?.content || "";
    const finishReason = String(choice?.finish_reason || "");
    const completionTokens = Number(item?.response?.body?.usage?.completion_tokens) || 0;
    ...
    try {
      ...
      // ⚠️ **被截断的生成结果一律不要**（这一条很重要）。
      //
      //    `finish_reason = "length"` 说明模型还没写完就被 max_tokens 掐断了。
      //    麻烦在于：**半截内容照样符合"标签/标题/正文"的格式**，
      //    于是解析成功、任务被标成 `pass`、用户看到一篇**戛然而止的日记** ——
      //    后台一切正常，只有用户能感觉到不对。
      //
      //    对一个心理陪伴产品来说，**"写着写着没了"的日记比没有日记更糟**（会让人不安）。
      //    所以宁可标 failed 让它重试，也不保存半截。
      //    （重试时输入已经被压短了，见 diary 分组的 maxChars / maxMessages）
      if (isGenerate && finishReason === "length") {
        failedCount += 1;
        await execute(
          "UPDATE content_review_tasks SET status = 'failed', reason = ?, provider = ?, reviewed_at = ? WHERE id = ?",
          [
            `生成被截断（输入 ${String(task.content || "").length} 字 / 输出 ${reply.length} 字${
              completionTokens ? ` / ${completionTokens} tokens` : ""
            }，finish_reason=length）—— 不保存半截日记，等下次自动重试`,
            "mimo-batch",
            now(),
            taskId,
          ]
      ...
      const parsed = isDiaryMood
        ? {
            verdict: pickMoodFromText(reply),
            reason: `AI 打标（finish_reason=${finishReason || "?"}）：${String(reply).slice(0, 80)}`,
          }
        : isGenerate
          ? {
              verdict: parseDiaryDraft(reply),
              // ⚠️ 把 finish_reason / 输出长度 / token 用量都写进原因 ——
              //    "生成结果为空"的时候，这一行就是**唯一的线索**。
              //    finish_reason=length 就说明是被 max_tokens 截断了。
              reason: `AI 生成（finish_reason=${finishReason || "?"}，输出 ${reply.length} 字${
                completionTokens ? ` / ${completionTokens} tokens` : ""
              }）：${String(reply).slice(0, 120)}`,
            }
          : parseVerdict(reply);
...
export async function applyManualVerdict({ taskId, approve, adminName }) {
  try {
    // ⚠️ 先看清这条任务是什么类型 —— **日记打标不能套用"通过 / 驳回"那一套**。
    //
    //    打标的结果是一个情绪词，本来就没有"合规不合规"可言。
    //    以前这里不分类型直接传 `verdict: "reject"`，结果这个字符串被写进了
    //    `diaries.mood`，用户的日记卡片上就出现了一个「reject」。
    const rows = await query(
      "SELECT id, user_id, field, target_id, task_kind FROM content_review_tasks WHERE id = ? LIMIT 1",
      [taskId]
    );
    if (!rows.length) return { ok: false, error: "任务不存在" };

    if (String(rows[0].task_kind || "") === "diary_mood") {
      if (approve) {
        await execute(
          "UPDATE content_review_tasks SET status = 'pass', reason = ?, provider = 'manual', reviewed_at = ? WHERE id = ?",
          [`管理员 ${adminName || ""} 人工确认了标签`.trim(), now(), taskId]
        );
        return { ok: true, verdict: "pass" };
      }

      // "驳回"在打标场景下的正确含义是：**这个标签不准，撤掉** ——
      // 而不是写一个假情绪词进用户的日记。
      await execute(
        "UPDATE content_review_tasks SET status = 'failed', reason = ?, provider = 'manual', reviewed_at = ? WHERE id = ?",
        [`管理员 ${adminName || ""} 认为标签不准，已撤回（日记可以重新排队打标）`.trim(), now(), taskId]
      );

      try {
        await execute("UPDATE diaries SET mood = NULL WHERE id = ? AND user_id = ?", [
          rows[0].target_id,
          rows[0].user_id,
        ]);
      } catch {
      ...
      return { ok: true, verdict: "withdrawn" };
    }

    if (!approve) {
      // 审核类：驳回 = 走和 AI 判定相同的处置逻辑（回填默认值）
...
export async function createDailyDiaryTasks() {
  ...
  for (const [userId, list] of byUser) {
    ...
    // ③ 去重：**同一天只要排过一次（或已经生成过），就不再排**。
    //
    //    ⚠️ 这里原来只查 `diaries` 表（"24 小时内是否生成过"），有个大漏洞：
    //    上一次生成**失败**时 `diaries` 里根本没有记录 → 去重不生效 →
    //    再点一次「为昨天排任务」就多一条，两条都跑完 → **两篇一模一样的日记**。
    //
    //    现在两道检查：
    //      ① **队列里同 key 还有活跃任务** → 跳过（这才是真正管用的那道）
    //      ② `diaries` 里已经有那一天的 AI 日记 → 跳过
    const nowBeijing = new Date(Date.now() + 8 * 3600 * 1000);
    nowBeijing.setUTCDate(nowBeijing.getUTCDate() - 1);
    // ⚠️ 按北京时间算 —— 服务器时区不可信（这条在压力评估那边也踩过）
    const diaryDate = nowBeijing.toISOString().slice(0, 10);
    const dedupeKey = `diary_generate:${userId}:${diaryDate}`;
    ...
    try {
      ...
      /* 老库还没补 dedupe_key 列时，靠 enqueueReview 内部那一层兜底 */
    }

    try {
      const exists = await query(
        `SELECT id FROM diaries WHERE user_id = ? AND source = 'ai' AND diary_date = ? LIMIT 1`,
        [userId, diaryDate]
      );
      if (exists.length) {
        already += 1;
