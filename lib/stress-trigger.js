...
export async function checkPopupTrigger(userId, finalScore, source = "chat") {
  ...
  if (Number(state.popup_enabled) === 0) return { allow: false, reason: "用户关闭了弹窗", state };

  const baseThreshold = Number(state.threshold) || Number(config?.stressThreshold) || DEFAULT_THRESHOLD;
  const threshold = source === "diary" ? Math.max(50, baseThreshold - DIARY_THRESHOLD_OFFSET) : baseThreshold;
  if (finalScore < threshold) {
    return { allow: false, reason: `未达阈值（${finalScore} < ${threshold}）`, state };
  }

  // 连续两次 —— 单次的尖峰不算
  const recent = await recentFinalScores(userId, 2);
  if (recent.length < 2 || recent.some((value) => value < threshold)) {
    return { allow: false, reason: "还没有连续两次达到阈值", state };
  }
  ...
  if (state.last_popup_at) {
    ...
    if (Number.isFinite(last) && now - last < cooldownMs) {
      return { allow: false, reason: "还在弹窗冷却期", state };
    }
  }
  ...
  if (Number(state.popup_reject_count || 0) > 0 && state.last_popup_at) {
    ...
    if (Number.isFinite(last) && now - last < TRIGGER_DEFAULTS.rejectCooldownMs) {
      return { allow: false, reason: "刚刚拒绝过，先不打扰", state };
    }
  }
  ...
  try {
    ...
    if (active.some((item) => Number(item.accepted) === 1 && Number(item.completed) === 0)) {
      return { allow: false, reason: "正在放松中", state };
    }
  } catch {
    /* 查不了就继续 */
  }

  return { allow: true, reason: "", state };
}
...
async function recentSessions(userId, limit = 1) {
  try {
    ...
    return await query(
      `SELECT id, accepted, completed FROM relaxation_sessions
