 ...
 * 阈值语义：**0 表示"跟随后台配置"**。
 *
 * ⚠️ 这里踩过一个坑：原来把默认值写成 70 直接存进用户表，
 *    于是管理员在后台把「提醒阈值」改成 60 —— **完全不生效**，
 *    因为 `Number(state.threshold) || config.stressThreshold` 永远先取到那个 70。
 *    现在的规则：用户没主动调过就是 0，读取时再回落到后台配置。
 */
export const THRESHOLD_FOLLOW_CONFIG = 0;

/** 用户自己也调过时的允许区间 */
export const MIN_USER_THRESHOLD = 40;
export const MAX_USER_THRESHOLD = 95;

/** 后台也没配时的兜底值 */
export const FALLBACK_THRESHOLD = 70;
 ...
 * 优先级：**用户自己调过的 > 后台配置 > 兜底**
 *
 * @param {number|string} stateThreshold `user_stress_state.threshold`（0 = 跟随）
 * @param {number|string} configThreshold 后台 `safety.stressThreshold`
 */
export function resolveThreshold(stateThreshold, configThreshold) {
  const clamp = (value) =>
    Math.max(MIN_USER_THRESHOLD, Math.min(MAX_USER_THRESHOLD, Math.round(value)));

  const userValue = Number(stateThreshold) || 0;
  if (userValue > 0) return clamp(userValue);

  const configValue = Number(configThreshold) || 0;
  if (configValue > 0) return clamp(configValue);
...
const STATE_DEFAULTS = {
  ...
  // ⚠️ 0 = 跟随后台配置（不是"阈值 0 分"）
  threshold: THRESHOLD_FOLLOW_CONFIG,
...
export async function getStressState(userId) {
  ...
  try {
    ...
    await execute(
      ...
      [id, BASE_SCORE, BASE_SCORE, BASE_SCORE, THRESHOLD_FOLLOW_CONFIG]
    );
